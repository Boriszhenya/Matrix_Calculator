/* Було виконано наступні зміни
   1. Стилістика написання кода
	   - назви функцій були написані з великої літери і використовувалися пробіли;
	   - в назвах функцій та змінних були граматичні помилки, що призводило до
		 складності в розумінні;
	   - в написані назв функцій використав "верблюжий шрифт";
	   - підправив кількіть пробілів;
	   - добавив коментарі та розмістив їх на однакових відстанях;
   2. В case 6: добавив умову на введення степеня до якого підноситься матриця - має бути >= 1,
	  також трохи оптимізував код.
 */
...
void exponentiationArray(double** arr1, double** arr2, double** arr_res, int rows, int power)
{
	...
	for (int k = 2; k <= power; k++)                           // Якщо степінь 2 і більше то виконувати 
															   //множення матриць.
	{
...
double determinant(double** arr, int k)
{
	...
	for (int l = 0; l < k; l++)
	{
		...
		if (l % 2)													   // Обчислюємо детермінант.
		{
			det += (-1) * arr[0][l] * determinant(Dyn_Minor_Arr, k - 1);
		}
		else
		{
			det += arr[0][l] * determinant(Dyn_Minor_Arr, k - 1);
		}
	}
...
void multiplicationByNumber(double** arr, int rows, int cols, double number)
{
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < cols; j++)
